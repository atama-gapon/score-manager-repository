	...
	<c:param name="content">
		<section class="me-4">
			...
			<form action="PositionCreateExecute.action" method="post" class="px-4">
				<label>役職名</label> <input type="text" name="name" class="form-control" value="${name}">
				<c:if test="${not empty errors.name}">
					<div class="col-12 mt-2 text-warning">${errors.name}</div>
				</c:if>
				<br> <label>表示順</label> <input type="number" name="sort_order" class="form-control" value="${sort_order}">
