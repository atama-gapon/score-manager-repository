...
<c:import url="/WEB-INF/jsp/common/base.jsp">
	...
	<c:param name="content">
		...
		<form action="StudentCreateExecute.action" method="post" class="px-4">
			...
			<!-- 入学年度 -->
			<label>入学年度</label> <select name="ent_year" class="form-select">
				<option value="">--------</option>
			...
			<!-- 学生番号（10文字まで） -->
			<label>学生番号</label> <input type="text" name="no" class="form-control"
				maxlength="10" value="${no}" placeholder="学生番号を入力してください" required>
			...
			<!-- 氏名30文字 -->
			<label>氏名</label> <input type="text" name="name" class="form-control"
				maxlength="30" value="${name}" placeholder="氏名を入力してください" required>
			...
			<div class="mb-3">
				<label class="form-label">クラス</label> <select name="class_num"
					...
					<c:forEach var="c" items="${class_num_set}">
						<option value="${c}"
							<c:if test="${c == class_num}">selected</c:if>>${c}</option>
					</c:forEach>
				</select>
			</div>
